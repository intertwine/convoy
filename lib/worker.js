...
var rcWorker = function(queue, job){
  this.queue = queue;
  this.client = queue.workerClient;
  if(job) { this.job = job; }
};

module.exports = rcWorker;

rcWorker.prototype.start = function(cb) {
  var self = this;
...
rcWorker.prototype.processing = function(cb) {
...
*/
rcWorker.prototype.notProcessing = function(cb) {
  this.client.multi()
...
*/
rcWorker.prototype.processed = function(err, cb) {
...
rcWorker.prototype.fail = function(jobError, cb) {
  var self = this;
  if(!cb) { cb = function(){}; }

  // Job has failed, remove it from processing and add it to failed
  this.notProcessing(function(){
    self.client.zincrby(helpers.key(self.queue.name+':failed'), 1, self.job.id, function(err){
...
rcWorker.prototype.logError = function(message, cb) {
